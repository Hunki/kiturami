...
async def async_setup_platform(hass, config, async_add_entities,
    ...
    slave_id = await krb_api.slave_id(node_id)
    room = []
    for i in slave_id:
        _LOGGER.info("register room" + i['alias'])
        room.append(Kiturami(name + " " + i['alias'], DeviceAPI(krb_api, node_id, i['slaveId'])))

    async_add_entities(room, True)
...
class KrbAPI:
    ...
    async def request(self, url, args):
        ...
        _LOGGER.debug('Request: %s, %s', url, args)
        try:
    ...
    async def slave_id(self, node_id):
...
class DeviceAPI:
    ...
    def __init__(self, krb, node_id, slave_id):
        """Initialize the Kiturami Member API.."""
        self.krb = krb
...
class Kiturami(ClimateEntity):
    ...
    def unique_id(self):
        """Return a unique ID."""
        return self.device.node_id + self.device.slave_id
    ...
    def device_info(self):
        ...
        return {
            "identifiers": {('kiturami', self.device.node_id +  + self.device.slave_id)},
            'name': 'Kiturami IOT',
            'manufacturer': 'kiturami',
            'model': 'NRS1200WF',
            'device_alias': self.result['deviceAlias'] + self.device.slave_id
        }
